...
*/
struct huffman *get_huffman_from_freq(uint8_t n, uint32_t *frequences)
{
    ...
    arbres[next_indice].suivant = NULL;
    ++next_indice;
    // On créé un tas min (pour le fréquence) de pointeurs vers des struct arbre
    ...
    while (taille_tas > 1) {
        ...
        // On remplace le petit arbre restant par une fusion des deux plus petit
...
struct huffman **get_huffman_from_mcu(struct array_mcu *mcu)
{
    ...
    retour[0] = get_huffman_from_freq(12, DC_Y_freq);
    retour[1] = get_huffman_from_freq(11, AC_Y_freq);
    ...
    if (mcu->ct == COLOR) {
        ...
        for (uint8_t i = 0; i < 12; ++i) {
            DC_Cb_freq[i] = ((DC_Cb_freq[i] + 1) >> 1) + ((DC_Cr_freq[i] + 1) >> 1);
        }
        for (uint8_t i = 1; i < 11; ++i) {
            AC_Cb_freq[i] = ((AC_Cb_freq[i] + 1) >> 1) + ((AC_Cr_freq[i] + 1) >> 1);
        }
        free(AC_Cr_freq);
        free(DC_Cr_freq);
        ...
        AC_Cb_freq[0] = 0;
        // On ajoute les arbres de huffman
        retour[2] = get_huffman_from_freq(12, DC_Cb_freq);
        retour[3] = get_huffman_from_freq(11, AC_Cb_freq);
        free(AC_Cb_freq);
        free(DC_Cb_freq);
    }
    return retour;
